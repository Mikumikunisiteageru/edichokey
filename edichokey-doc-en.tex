...
A package option named $hyperref$ is provided to make the output key easier to use on electronic devices (type $\string\usepackage[hyperref]\{edichokey\}$ instead). When this option is loaded, every alternative will link to its conjugate one by clicking the number, however, the source needs to be compiled twice.
